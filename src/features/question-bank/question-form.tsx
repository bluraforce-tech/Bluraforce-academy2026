...
export function BankQuestionForm({unitId}:{unitId:string}){
 ...
 return <form action={addBankQuestion} className="bank-question-form" onSubmit={e=>{const f=new FormData(e.currentTarget);(e.currentTarget.elements.namedItem("payload") as HTMLInputElement).value=JSON.stringify({unitId,text:f.get("text"),imageUrl:f.get("imageUrl"),points:Number(f.get("points")),choices})}}>
  ...
  {choices.map((c,i)=><div className="choice-row" key={i}><input value={c.text} onChange={e=>setChoices(v=>v.map((x,j)=>j===i?{...x,text:e.target.value}:x))} placeholder={`Choice ${i+1}`} required/><label><input type="checkbox" checked={c.isCorrect} onChange={e=>setChoices(v=>v.map((x,j)=>j===i?{...x,isCorrect:e.target.checked}:x))}/>Correct</label>{choices.length>2&&<button type="button" onClick={()=>setChoices(v=>v.filter((_,j)=>j!==i))}><Trash2/></button>}</div>)}
