...
const question=z.object({text:z.string().trim().min(1).max(5000),imageUrl:z.union([z.literal(""),z.string().url()]),imageGroupIndex:z.number().int().nonnegative().optional(),points:z.number().positive().max(1000),position:z.number().int().positive(),choices:z.array(choice).min(2).refine(v=>v.some(c=>c.isCorrect))});
...
export async function createExam(formData:FormData){
 ...
 let questionsWithImages=parsed.data.questions;try{const groupIds=[...new Set(questionsWithImages.flatMap(q=>q.imageGroupIndex===undefined?[]:[q.imageGroupIndex]))];const groupImages=new Map<number,string>();for(const groupId of groupIds){const image=await uploadQuestionImage(supabase,user.id,formData.get(`questionGroupImage_${groupId}`));if(!image)redirect("/teacher/exams/new?error=image");groupImages.set(groupId,image)}questionsWithImages=await Promise.all(questionsWithImages.map(async(question,index)=>{const individualImage=await uploadQuestionImage(supabase,user.id,formData.get(`questionImage_${index}`));return {...question,imageUrl:individualImage??(question.imageGroupIndex===undefined?question.imageUrl:groupImages.get(question.imageGroupIndex)!)};}))}catch{redirect("/teacher/exams/new?error=image")}
