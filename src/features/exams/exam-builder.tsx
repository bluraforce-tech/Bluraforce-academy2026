...
export function ExamBuilder({
  ...
  const changeQ = (i: number, patch: Partial<Question>) =>
    setQuestions((value) =>
      value.map((question, index) =>
        index === i ? { ...question, ...patch } : question,
      ),
    );
  const changeC = (qi: number, ci: number, patch: Partial<Choice>) =>
    setQuestions((value) =>
      value.map((question, index) =>
        index === qi
          ? {
              ...
              choices: question.choices.map((choice, choiceIndex) =>
                choiceIndex === ci ? { ...choice, ...patch } : choice,
              ),
            }
          : question,
      ),
  ...
  const toggleBankPage = (page: (typeof bankPages)[number]) => {
    const pageSelected = page.questions.every((question) =>
      selectedSources.has(question.sourceId),
    );
    if (pageSelected)
      setQuestions((value) =>
        value.filter(
          (question) =>
            !page.questions.some((item) => item.sourceId === question.sourceId),
        ),
      );
    else
      setQuestions((value) => {
        ...
        const additions = page.questions
          .filter((question) => !existing.has(question.sourceId))
          .map((question, index) => ({
            text: question.text,
            imageUrl: question.imageUrl,
            ...
            questionNumber: value.length + index + 1,
          }));
      ...
      onSubmit={(event) => {
        ...
        const payload = {
          ...
          questions: questions.map((question, index) => ({
            ...
            choices: question.choices.map((choice, choiceIndex) => ({
              ...choice,
              text: choice.text.trim() || String.fromCharCode(65 + choiceIndex),
    ...
    >
      ...
      <section className="builder-overview" aria-label="Exam overview">
        ...
        <div>
          ...
          <span>
            ...
            <small>Default duration</small>
          </span>
        </div>
        <div>
          <Users />
          <span>
            <strong>{assignAll ? "All" : "Selected"}</strong>
      ...
      <section className="panel form-grid exam-settings">
        <div className="form-section-title full">
          ...
          <div>
            <h2>Exam details</h2>
            <p>Set the schedule, scoring, and student access.</p>
          </div>
        </div>
        <div className="field">
          <label>Title</label>
      ...
      {questionBank.length > 0 && (
        <section className="panel question-bank">
          <div className="panel-head">
            <div>
              <h2>Old exam pages</h2>
              <p>
                Select a complete page to reuse its uploaded image and all
                questions.
              </p>
            </div>
          </div>
          <div className="question-bank-pages">
            {bankPages.map((page) => {
              const selected = page.questions.every((question) =>
                selectedSources.has(question.sourceId),
              );
              return (
                <article className={selected ? "selected" : ""} key={page.id}>
                  <button
                    type="button"
                    className="bank-page-select"
                    onClick={() => toggleBankPage(page)}
                    aria-pressed={selected}
                  >
                    <span className="bank-page-check">
                      {selected ? <CheckCircle2 /> : <Plus />}
                    </span>
                    <span>
                      <b>{selected ? "Page added" : "Add this page"}</b>
                      <small>
                  ...
                  {page.imageUrl ? (
                    <div className="bank-page-image">
                      <img
                        src={page.imageUrl}
                        alt={`Page from ${page.title}`}
                      />
                    </div>
                  ) : (
                    <div className="bank-page-image empty">
                      No uploaded page image
                    </div>
                  )}
                  <div className="bank-page-details">
                    <strong>{page.title}</strong>
                    {page.questions.map((question, index) => (
                      <div key={question.sourceId}>
                        <span>{question.questionNumber || index + 1}</span>
                        <p>{question.text}</p>
                      </div>
                    ))}
                  </div>
      ...
      <div className="builder-heading">
        <div>
          ...
          <p>
            Upload a page, then add all of its questions directly underneath.
          </p>
        </div>
      </div>
      {imageGroups.map((groupId, pageIndex) => (
        <section className="panel question-page-group" key={groupId}>
          <div className="question-page-heading">
            <div>
              <small>QUESTION PAGE {pageIndex + 1}</small>
              <h2>Upload image, then add its questions</h2>
            </div>
            {imageGroups.length > 1 && (
              <button
                ...
                onClick={() => {
                  setImageGroups((v) => v.filter((id) => id !== groupId));
                  setQuestions((v) =>
                    v.filter((q) => q.imageGroupIndex !== groupId),
                  );
                }}
              >
                <Trash2 />
          ...
          <div className="questions-under-image">
            {questions.map(
              (question, questionIndex) =>
                question.imageGroupIndex === groupId && (
                  <section className="question-editor" key={questionIndex}>
                    <div className="question-top">
                      <label className="question-number-input">
                      ...
                      {questions.length > 1 && (
                        <button
                          type="button"
                          aria-label={`Delete question ${question.questionNumber ?? questionIndex + 1}`}
                          onClick={() =>
                            setQuestions((value) =>
                              value.filter(
                    ...
                    <div className="choice-tools">
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          changeQ(questionIndex, { choices: abcd() })
                        }
                      >
                      ...
                      {question.choices.map((choice, choiceIndex) => (
                        <div className="choice-row" key={choiceIndex}>
                          ...
                          <input
                            value={choice.text}
                            onChange={(event) =>
                              changeC(questionIndex, choiceIndex, {
          ...
          <button
            ...
            onClick={() =>
              setQuestions((v) => [
                ...v,
                { ...blank(v.length + 1), imageGroupIndex: groupId },
              ])
            }
          >
            <Plus />
      ...
      <div className="exam-page-actions">
        <button
          type="button"
          className="button secondary add-image-bottom"
          onClick={() => {
            const id = Math.max(-1, ...imageGroups) + 1;
            setImageGroups((v) => [...v, id]);
            setQuestions((v) => [
              ...v,
              { ...blank(v.length + 1), imageGroupIndex: id },
            ]);
          }}
        >
          <Plus />
        ...
        <button
          type="button"
          className="button exam-preview-launch"
          onClick={() => setPreviewOpen(true)}
        >
          <Eye />
          Preview full student exam
        </button>
      </div>
        ...
        >
          <div className="exam-preview-shell">
            <header className="exam-preview-header">
              <div>
                <small>STUDENT VIEW · LIVE PREVIEW</small>
                <h1>{title.trim() || "Untitled exam"}</h1>
                <p>
            ...
            <main className="exam-preview-content">
              {imageGroups.map((groupId, pageIndex) => (
                <section className="exam-preview-page" key={groupId}>
                  <div className="preview-page-label">
                    <span>Page {pageIndex + 1}</span>
                    <small>
                      {
                        questions.filter(
                          (question) => question.imageGroupIndex === groupId,
                        ).length
                      }{" "}
                      questions
                    </small>
                  ...
                  <div className="preview-question-list">
                    {questions
                      ...
                      .map((question, index) => (
                        <article
                          className="panel preview-question"
                          key={`${groupId}-${index}`}
                        >
                          <div>
              ...
              {questions.some(
                (question) => question.imageGroupIndex === undefined,
              ) && (
                <section className="exam-preview-page">
                  <div className="preview-page-label">
      ...
      {questions.some((q) => q.imageGroupIndex === undefined) && (
        <>
          <div className="builder-heading">
            <h2>Questions from your bank</h2>
          </div>
          {questions.map(
            (question, questionIndex) =>
              question.imageGroupIndex === undefined && (
                <section className="panel question-editor" key={questionIndex}>
                  <div className="question-top">
