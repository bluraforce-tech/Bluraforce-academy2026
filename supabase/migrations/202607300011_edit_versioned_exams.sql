...
begin
 ...
 update exams set title=trim(p_payload->>'title'),description=nullif(trim(p_payload->>'description'),''),instructions=nullif(trim(p_payload->>'instructions'),''),
 duration_minutes=(p_payload->>'durationMinutes')::int,starts_at=nullif(p_payload->>'startsAt','')::timestamptz,ends_at=nullif(p_payload->>'endsAt','')::timestamptz,
 status=case when coalesce((p_payload->>'publish')::boolean,false) then 'published'::record_status else 'draft'::record_status end,
 max_attempts=(p_payload->>'maxAttempts')::int,randomize_questions=coalesce((p_payload->>'randomizeQuestions')::boolean,false),
 randomize_choices=coalesce((p_payload->>'randomizeChoices')::boolean,false),updated_at=now() where id=p_exam_id;
 ...
 for q in select * from jsonb_array_elements(p_payload->'questions') loop
  ...
  insert into questions(exam_id,text,image_url,points,position) values(p_exam_id,trim(q->>'text'),nullif(trim(q->>'imageUrl'),''),(q->>'points')::numeric,(q->>'position')::int) returning id into q_id;
 ...
 if coalesce((p_payload->>'publish')::boolean,false) then
  ...
  select jsonb_build_object('examId',p_exam_id,'title',e.title,'description',e.description,'instructions',e.instructions,'durationMinutes',e.duration_minutes,'scoringMode',e.scoring_mode,
   'questions',jsonb_agg(jsonb_build_object('id',qr.id,'text',qr.text,'imageUrl',qr.image_url,'points',qr.points,'position',qr.position,
    'choices',(select jsonb_agg(jsonb_build_object('id',cr.id,'text',cr.text,'isCorrect',cr.is_correct,'position',cr.position) order by cr.position) from question_choices cr where cr.question_id=qr.id)) order by qr.position))
  into snapshot from exams e join questions qr on qr.exam_id=e.id where e.id=p_exam_id group by e.id;
  ...
  if coalesce((p_payload->>'assignAll')::boolean,false) then
   insert into exam_assignments(exam_id,student_id) select p_exam_id,en.student_id from teacher_student_enrollments en where en.teacher_id=auth.uid() and en.status='active' and (en.access_expires_at is null or en.access_expires_at>now()) on conflict do nothing;
  else
   for student in select (value #>> '{}')::uuid from jsonb_array_elements(p_payload->'studentIds') loop
    ...
    insert into exam_assignments(exam_id,student_id) values(p_exam_id,student) on conflict do nothing;
   end loop;
  end if;
 end if;
