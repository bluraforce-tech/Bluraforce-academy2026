...
const question=z.object({text:z.string().trim().max(5000),imageUrl:z.union([z.literal(""),z.string().url()]),imageGroupIndex:z.number().int().nonnegative().optional(),points:z.number().positive().max(1000),position:z.number().int().positive(),choices:z.array(choice).min(2).refine(v=>v.some(c=>c.isCorrect))});
