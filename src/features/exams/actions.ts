...
export async function updateExam(_previous:ExamUpdateState,formData:FormData):Promise<ExamUpdateState>{
 ...
 let raw:unknown;try{raw=JSON.parse(String(formData.get("payload")??""))}catch{return {error:"The edited exam data could not be read. Your changes are still here; please try saving again."}}
