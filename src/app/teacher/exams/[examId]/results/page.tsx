...
export default async function Page({params}:{params:Promise<{examId:string}>}){
 ...
 const [{data,error},{data:source}]=await Promise.all([supabase.rpc("get_teacher_exam_results",{p_exam_id:examId}),supabase.from("exams").select("kind,source_unit_id").eq("id",examId).eq("teacher_id",user.id).single()]);if(error||!data)redirect("/teacher/exams");const exam=data as Results;
