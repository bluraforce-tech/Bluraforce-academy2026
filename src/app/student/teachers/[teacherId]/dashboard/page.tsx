...
export default async function TeacherPortal({params,searchParams}:{params:Promise<{teacherId:string}>;searchParams:Promise<{joined?:string;category?:string}>}) {
  ...
  const cards=[{key:"exams",label:"Assigned exams",count:portal.exams.length,text:"Open assessments and results",icon:BookOpen},{key:"mistakes-exams",label:"Mistakes exams",count:mistakes.length,text:"Revision exams generated from your mistakes",icon:Brain},{key:"activities",label:"Practice & Homework",count:activities.length,text:"Untimed activities and assigned homework",icon:Library},{key:"videos",label:"Lesson videos",count:portal.videos.length,text:"Lessons available from this teacher",icon:PlayCircle},{key:"materials",label:"Material Books",count:portal.materials.length,text:"Books, notes, worksheets, and files",icon:FileText}] as const;
