...
export default async function Page({
 ...
 searchParams:Promise<{teacherId?:string;category?:string}>;
}){
 ...
 const validCategory=category==="classified"||category==="sat"||category==="est";
 const backHref=validTeacherId?`/student/teachers/${teacherId}/materials${validCategory?`?category=${category}`:""}`:"/student/teachers";
 ...
 return <main className="app-content material-reader-page">
  ...
  <MaterialBookViewer assignmentId={assignmentId} url={data.external_url} title={data.title} studentName={profile?.full_name??"Student"}/>
 </main>;
}
