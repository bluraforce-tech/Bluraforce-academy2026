...
begin
 ...
 if v_completed>=v_checkpoint+v_interval then
  ...
  if v_count>0 then
   ...
   -- Generated mistakes exams require explicit teacher approval per student.
   insert into exam_assignments(exam_id,student_id,revoked_at) values(v_exam,v_student,now());
