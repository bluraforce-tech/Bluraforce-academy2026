alter table public.materials
  add column if not exists resource_kind text not null default 'material_book';

alter table public.materials drop constraint if exists materials_resource_kind_check;
alter table public.materials add constraint materials_resource_kind_check
  check (resource_kind in ('material_book','study_note'));

create index if not exists materials_teacher_kind_status_idx
  on public.materials(teacher_id,resource_kind,status);
